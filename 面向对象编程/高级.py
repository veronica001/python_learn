...
'''
定制类
__str__:只打印实例对象时，可返回自定义内容，返回用户看到的
__repr__：返回程序开发者看到的，为调试用的，通常__repr__ = __str__
...
a = A('test')
print(a) #在命令行打印仍然是显示地址，这是因为直接显示变量调用的不是__str__()，而是__repr__()，两者的区别是__str__()返回用户看
...
'''
__getattr__:动态返回一个属性，可以针对完全动态的情况作调用。在访问对象的属性的时候，如果对象并没有这个相应的属性，方法，那么将会调用
这个方法来处理   eg: s.name
__setattr__:当试图对象的没有的属性赋值的时候将会被调用  eg: s.name = 1
__call__: 一个对象实例可以有自己的属性和方法，当我们调用实例方法时，我们用instance.method()来调用,通过__call__可以直接对
实例进行调用
...
# 利用__getattr__，可以写一个链式调用

class Chain(object):
    def __init__(self, path=''):
        print('1.init path %s' % path)
        self.__path = path
    def __getattr__(self, path):
        print('2.getattr path is %s' % path)
        return Chain('%s/%s' %(self.__path, path))
    def __call__(self, param):
        print('3.call param is %s' % param)
        return Chain('%s/%s' % (self.__path, param))
    def __str__(self):
        return self.__path
    __repr__ = __str__

print(Chain().users.timelint.list)
print(Chain().users("test").time.list)